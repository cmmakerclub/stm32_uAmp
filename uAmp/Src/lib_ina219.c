...
{
//	HAL_I2C_Mem_Write(&hi2c1, 0x50, (uint16_t)reg, 1, (uint8_t*)value, 2,5);
}
...
{
	HAL_I2C_Mem_Read(&hi2c1, ina219_i2caddr, (uint16_t)reg, 1, (uint8_t*)value, 2,5);
}
...
void ina219_SetCalibration_32V_2A(void)
{
  ...
  ina219_currentDivider_mA = 10/2;  // Current LSB = 100uA per bit (1000/100 = 10)
  ina219_powerDivider_mW = 2/2;     // Power LSB = 1mW per bit (2/1)

  // Set Calibration register to 'Cal' calculated above	
  ina219_wireReadRegister(INA219_REG_CALIBRATION, &ina219_calValue);
  ...
  ina219_wireReadRegister(INA219_REG_CONFIG, &config);
}
...
void ina219_SetCalibration_32V_1A(void)
{
  ...
  // Set multipliers to convert raw current/power values
  ina219_currentDivider_mA = 25/2;      // Current LSB = 40uA per bit (1000/40 = 25)
  ina219_powerDivider_mW = 1/2;         // Power LSB = 800?W per bit
...
void ina219_SetCalibration_16V_400mA(void) {
  ...
  // Set multipliers to convert raw current/power values
  ina219_currentDivider_mA = 20/2;  // Current LSB = 50uA per bit (1000/50 = 20)
  ina219_powerDivider_mW = 1/2;     // Power LSB = 1mW per bit
...
{
  uint16_t value;
  ina219_wireWriteRegister(INA219_REG_SHUNTVOLTAGE, &value);
...
int16_t ina219_getCurrent_raw(void) 
{
  ...
  uint16_t value;
  ina219_wireWriteRegister(INA219_REG_CURRENT, &value);
